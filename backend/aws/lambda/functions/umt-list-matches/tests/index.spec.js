...
describe('Test AWS Lambda: umt-list-matches', () => {
  ...
  test('Evaluar respuesta: Equipo (MAN. UNITED) Pt 1', (done) => {
    ...
    lambda.invoke(params, function(err, data) {
      if (err) {
        ...
        expect(response.nextToken).toBe('{"rangeKey":{"S":"MATCH#fcbarcelona"},"hashKey":{"S":"MATCH#man.united"}}&{"rangeKey":{"S":"MATCH#man.united"},"hashKey":{"S":"MATCH#bayern"}}')
      }
  ...
  test('Evaluar respuesta: Equipo (MAN. UNITED) Pt 2', (done) => {
    ...
    lambda.invoke(params, function(err, data) {
      if (err) {
        ...
        expect(data.StatusCode).toBe(200)
        expect(response.items[0]).toStrictEqual({teamId1: 'chelsea', teamId2: 'man.united'})
        expect(response.nextToken).toBe('{"rangeKey":{"S":"MATCH#psg"},"hashKey":{"S":"MATCH#man.united"}}&{"rangeKey":{"S":"MATCH#man.united"},"hashKey":{"S":"MATCH#chelsea"}}')
      }
  ...
  test('Evaluar respuesta: Equipo (MAN. UNITED) Pt 3', (done) => {
    ...
    lambda.invoke(params, function(err, data) {
      if (err) {
        ...
        expect(response.nextToken).toBe('{"rangeKey":{"S":"MATCH#realmadrid"},"hashKey":{"S":"MATCH#man.united"}}&{"rangeKey":{"S":"MATCH#man.united"},"hashKey":{"S":"MATCH#rpc"}}')
      }
